...
class NewInterruptionHandler(AbstractInterruptionHandler):
    def execute(self, irq):
        ...
        pcb = PCB(pid, baseDir, size, program.name, priority)
        self.kernel.pcbTable.add(pcb)

        log.logger.info("-- NEW PROGRAM {name}, PRIORITY {priority}".format(name=pcb.path, priority=pcb.priority))
...
class KillInterruptionHandler(AbstractInterruptionHandler):
    def execute(self, irq):
        ...
        self.kernel._loader.memoryManager.freeBlock(finished_pcb.baseDir, finished_pcb.size)
